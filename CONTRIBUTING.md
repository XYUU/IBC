...
1. **Discuss it first**: it's wise to raise an issue on the
   [GitHub repository](https://github.com/XYUU/IBC) explaining what you
...
4. **Non-developer users**: a lot of IBC users aren't developers. It must be
   ...
   that the [configuration file](resources/config.properties) and/or
...
5. **Backward compatibility**: new versions should not force users of previous
   ...
   introduce new settings into the [configuration file](resources/config.properties)
...
7. **Style**: please maintain the same style as the existing code unless
   ...
   GitHub [Issue](https://github.com/XYUU/IBC/issues)
...
The repository includes a [build.gradle](build.gradle) file that defines the
build process. This build process expects to find an `IB_HOME` environment
...
version of IBC, you'll find it helpful to set the `IB_HOME` environment
...
Note that the `ver` property in the [build.gradle](build.gradle) is used in
