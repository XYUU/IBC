...
class ConfigureTwsApiPortTask implements ConfigurationAction{
    ...
    private static final Logger logger = LoggerFactory.getLogger(ConfigureTwsApiPortTask.class);
    ...
    public void run() {
        try {
            logger.info("Performing port configuration");

            Utils.selectApiSettings(configDialog);

            // [AST重构审查] 来源Jar: jars/jts4launch-1045.jar | 规则: 完全匹配(区分大小写) | 相似度: 100.0%
            Component comp = SwingUtils.findComponentByBundle(configDialog, "ji18n.Language", "Configuration_Property4");
            ...
            if (currentPort == portNumber) {
                logger.info("TWS API socket port is already set to {}", tf.getText());
            } else {
                ...
                logger.info("TWS API socket port was set to {}", tf.getText());
                tf.setText(Integer.toString(portNumber));
                logger.info("TWS API socket port now set to {}", tf.getText());
            }
